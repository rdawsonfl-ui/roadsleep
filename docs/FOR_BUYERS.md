...
- **1,980 lodging listings** (1,616 hotels + 364 RV parks) across 13 active interstate corridors east of the Mississippi
- **692 mapped exits** with mile markers and lat/lng coordinates
...
| Item | Status |
|---|---|
| Verified inventory | 1,405 of 1,980 listings verified against Google Places or by phone. 297 are hidden from drivers (closed, no working phone, or failed verification). A `show_unverified_to_drivers` setting controls whether the unverified remainder is live. |
| Recurring revenue | $0/mo. Three hoteliers signed up; none has been billed. No payment processor is connected. |
...
A scraper could assemble 1,980 hotel rows in a weekend. The reason this asset is worth more than that:
...
**2. Real geographic structure.** The 692 mapped exits with mile markers, lat/lng, and corridor associations are the durable asset. Even if every hotel listing was thrown out, the exit graph is the platform.
...
- [ ] What's the cost to verify (call + confirm) the existing 1,980 listings?
...
**Thin usage data.** 76 call-log rows. That's not a meaningful sample for valuing the platform's actual driver appeal. A 90-day pre-acquisition observation period would tell a buyer far more than the asset itself can today.

**Inventory data quality partially known.** Listings were sourced from Google
Places and OpenStreetMap. 1,405 of 1,980 have since been checked against Google
Places or by phone, and 297 were hidden as closed, unreachable, or failed. The
remaining unverified listings still carry the original sourcing risk — wrong
phone numbers, closed businesses, stale addresses.
